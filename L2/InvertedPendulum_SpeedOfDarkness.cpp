...
char have_set_constants = 0;
double error_angle_prev = 0;
double error_angle = 0;
double error_angle_total = 0;
double error_angle_diff = 0;
double error_velt_prev = 0;
double error_velt = 0;
double error_velt_total = 0;
double error_velt_diff = 0;
// Make the constants relative to the weight of the cart and the bar?
double k1 = 4000;
double k2 = 0;
double k3 = 2000;
double velt_k1 = 3500;
...
void ApplyHorizontalForce(void) {
	...
	*/
	...
	F=0;
	if (have_set_constants == 0) {
		// Modify PID constants to be relative of the mass of the bar
		k1 *= m/m1 * (m + m1)/200;
		k2 *= m/m1 * (m + m1)/200;
		k3 *= m/m1 * (m + m1)/200;
		velt_k1 *= m/m1 * (m + m1)/200;
		velt_k2 *= m/m1 * (m + m1)/200;
		velt_k3 *= m/m1 * (m + m1)/200;
		have_set_constants = 1;
	}
